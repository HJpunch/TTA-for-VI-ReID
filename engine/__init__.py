...
def get_trainer(dataset, model, optimizer, lr_scheduler=None, logger=None, writer=None, non_blocking=False, log_period=10,
    ...
    # 저장 주기: None/0/음수면 매 에폭 저장
    every = int(eval_interval) if (eval_interval is not None and int(eval_interval) > 0) else 1
    ...
    def _save(engine):
        # start_eval 이전엔 저장하지 않음
        if start_eval is not None and engine.state.epoch < int(start_eval):
            return
        to_save = {"model": model}
        ...
        checkpointer(engine, to_save)

    # 에폭 단위 저장 (iters 단위로 저장하고 싶으면 ITERATION_COMPLETED로 바꾸세요)
    trainer.add_event_handler(Events.EPOCH_COMPLETED(every=every), _save)
