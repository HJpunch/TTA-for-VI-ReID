...
from .sampling import assign_gallery_pseudo_label
...
def adapt_module(
    ...
    for bi, batch in enumerate(online_loader):
        ...
        pseudo_labels = assign_gallery_pseudo_label(F.normalize(gallery_feats, dim=1), threshold=0.85)
        triplet_loss = 0.0
        loss = 0.0
        ...
        with autocast(enabled=amp, device_type='cuda' if 'cuda' in device else 'cpu'):
            ...
            if u.numel() < 2:
                triplet_loss = 0.0 * gallery_feats.sum()  # 그래프 유지용
            
            else:
                triplet_loss, *_ = triplet_loss_function(gallery_feats, pseudo_labels)
        loss = triplet_loss
